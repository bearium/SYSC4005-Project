...
import (
	...
	"github.com/SYSC4005-Project/Version1/data"
	"github.com/SYSC4005-Project/Version1/pkg/component"
	"github.com/SYSC4005-Project/Version1/pkg/inspector"
...
func runBenchMark(i int) {
	...
	for {
		if i1.Blocked && i2.Blocked && w1.Blocked && w2.Blocked && w3.Blocked {
			...
			var systemTotalLambda float64
			var systemTotalL float64
			var systemTotalW float64
			var TotalAverage float64
			fmt.Println("Inspector 1:")
			arivalRate := float64(i1.TotalComponentS) / float64(i1.TotalArrivalTIme.Seconds())
			fmt.Printf("Lambda= %v/s\n", arivalRate)
			systemTotalLambda = systemTotalLambda + arivalRate
			totalTime := float64(i1.ClosedTime.Sub(start).Seconds())
			w := float64(totalTime) / float64(i1.TotalComponentS)
			...
			arivalRate = float64(i2.TotalComponentS) / float64(i2.TotalArrivalTIme.Seconds())
			fmt.Printf("Lambda= %v/s\n", arivalRate)
			systemTotalLambda = systemTotalLambda + arivalRate
			...
			for i := 0; i < len(w1.QueueData.ItemsInQueue); i++ {
				average := float64(w1.QueueData.ItemsInQueue[i].Seconds()) / float64(w1.ClosedTime.Sub(w1.StartTime).Seconds())
			...
			fmt.Printf("L= %v\n", TotalAverage)
			systemTotalL = systemTotalL + TotalAverage
			arivalRate = (float64(i1.TotalComponentS) + float64(i2.TotalComponentS)) / float64(elapsed.Seconds())
			fmt.Println("SYSTEM TOTAL:")
			fmt.Printf("Lambda= %v/s\n", arivalRate)
			w = float64(elapsed.Seconds()) / (float64(w3.TotalProduced) + float64(w1.TotalProduced) + float64(w2.TotalProduced))
			fmt.Printf("w=%v\n", w)
			fmt.Printf("lambda*w= %v\n", arivalRate*w)
			fmt.Printf("L= %v\n", systemTotalL)
			time.Sleep(1000)
